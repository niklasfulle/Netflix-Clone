...
export async function GET() {
  try {
    ...
    if (!user) {
      logBackendAction('api_movies_route_no_user', {}, 'error');
    ...
    if (!profil) {
      logBackendAction('api_movies_route_no_profil', { userId: user.id }, 'error');
      return Response.json(null, { status: 404 })
    }
    ...
    logBackendAction('api_movies_route_error', { error: String(error) }, 'error');
    console.log(error)
