    ...
    <div>
      ...
      <section className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/50">
        <div className={`grid gap-3 border-b border-zinc-800 p-4 ${sourceDetail.filterGrid}`}>
          ...
          {source === "container" ? null : <input aria-label="Nach Aktion filtern" value={action} onChange={(event) => { setAction(event.target.value); setPage(1); }} placeholder="Aktion …" className="h-10 rounded-lg border border-zinc-700 bg-zinc-950 px-3 text-sm text-white" />}
