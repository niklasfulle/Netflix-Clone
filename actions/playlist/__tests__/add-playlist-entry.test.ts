...
describe('add playlist entry action - Authentifizierung & Validierung', () => {
  ...
  it('❌ sollte Fehler zurückgeben wenn playlistId ungültig ist', async () => {
    ...
    expect(db.playlistEntry.findMany).not.toHaveBeenCalled();
    expect(db.playlistEntry.create).not.toHaveBeenCalled();
  });
  ...
  it('❌ fügt keinen Eintrag zu einer fremden Playlist hinzu', async () => {
    const mockCurrentUser = currentUser as jest.MockedFunction<typeof currentUser>;
    mockCurrentUser.mockResolvedValue({ id: 'user1' } as any);
    (db.profil.findFirst as jest.Mock).mockResolvedValue({ id: 'profil1' });
    (db.playlist.findFirst as jest.Mock).mockResolvedValue(null);

    const result = await addPlaylistEntry({ playlistId: 'foreign', movieId: 'movie123' });
