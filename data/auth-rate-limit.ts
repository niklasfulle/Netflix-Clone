...
export const authRateLimitRepository: AuthRateLimitRepository = {
  async consume(inputs) {
    ...
    return db.$transaction(async transaction => {
      const buckets = [];
      for (const { scope, subjectType, subjectHash, limit, windowMs, now } of inputs) {
        const timestamp = utcTimestamp(now);
        const nextResetAt = utcTimestamp(new Date(now.getTime() + windowMs));
        const rows = await transaction.$queryRaw<Array<{ attempts: number; resetAt: Date }>>(Prisma.sql`
      ...
      const cleanupTimestamp = utcTimestamp(inputs[0].now);
      await transaction.$executeRaw(Prisma.sql`
        DELETE FROM "AuthRateLimit"
        WHERE ctid IN (
          SELECT ctid
          FROM "AuthRateLimit"
          WHERE "resetAt" <= ${cleanupTimestamp}
  ...
  async reset(subjects) {
    ...
    await db.$transaction(async transaction => {
      for (const { scope, subjectType, subjectHash } of subjects) {
        await transaction.$executeRaw(Prisma.sql`
          DELETE FROM "AuthRateLimit"
          WHERE "scope" = ${scope}
