...
describe('Password Reset Token Data Access', () => {
  const filePath = path.join(process.cwd(), 'data/password-reset-token.ts');
  ...
  describe('Basic File Structure', () => {
    ...
    it('should export two functions', () => {
      expect(fileContent).toContain('export const getPasswordResetTokenByToken');
      expect(fileContent).toContain('export const getPasswordResetTokenByEmail');
    });
  ...
  describe('getPasswordResetTokenByToken Function', () => {
    it('should export getPasswordResetTokenByToken', () => {
      expect(fileContent).toContain('export const getPasswordResetTokenByToken');
    });

    it('should be an async function', () => {
      expect(fileContent).toMatch(/export const getPasswordResetTokenByToken = async/);
    });

    it('should accept token parameter', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByToken[\s\S]*?\(\s*token:\s*string/);
    });

    it('should have token parameter of type string', () => {
      const match = fileContent.match(/getPasswordResetTokenByToken[\s\S]*?\(\s*token:\s*string\s*\)/);
      expect(match).not.toBeNull();
    });

    it('should call db.passwordResetToken.findUnique', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByToken[\s\S]*?db\.passwordResetToken\.findUnique/);
    });
    ...
    it('should await the query', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByToken[\s\S]*?await.*findUnique/);
    });

    it('should return password reset token', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByToken[\s\S]*?return passwordResetToken/);
    });

    it('should have try-catch error handling', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByToken[\s\S]*?try[\s\S]*?catch/);
    });

    it('should return null on error', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByToken[\s\S]*?catch[\s\S]*?return null/);
    });
  });

  describe('getPasswordResetTokenByEmail Function', () => {
    it('should export getPasswordResetTokenByEmail', () => {
      expect(fileContent).toContain('export const getPasswordResetTokenByEmail');
    });

    it('should be an async function', () => {
      expect(fileContent).toMatch(/export const getPasswordResetTokenByEmail = async/);
    });

    it('should accept email parameter', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByEmail[\s\S]*?\(\s*email:\s*string/);
    });

    it('should have email parameter of type string', () => {
      const match = fileContent.match(/getPasswordResetTokenByEmail[\s\S]*?\(\s*email:\s*string\s*\)/);
      expect(match).not.toBeNull();
    });

    it('should call db.passwordResetToken.findFirst', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByEmail[\s\S]*?db\.passwordResetToken\.findFirst/);
    });
    ...
    it('should await the query', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByEmail[\s\S]*?await.*findFirst/);
    });

    it('should return password reset token', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByEmail[\s\S]*?return passwordResetToken/);
    });

    it('should have try-catch error handling', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByEmail[\s\S]*?try[\s\S]*?catch/);
    });

    it('should return null on error', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByEmail[\s\S]*?catch[\s\S]*?return null/);
    });
  });

  describe('Database Query Methods', () => {
    it('should use findUnique for token lookup', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByToken[\s\S]*?findUnique/);
    });

    it('should use findFirst for email lookup', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByEmail[\s\S]*?findFirst/);
    });

    it('should query passwordResetToken table', () => {
      expect(fileContent).toContain('db.passwordResetToken');
    });
  ...
  describe('Parameter Handling', () => {
    it('should accept token parameter in first function', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByToken[\s\S]*token:\s*string/);
    });

    it('should accept email parameter in second function', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByEmail[\s\S]*email:\s*string/);
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should return null on error for token function', () => {
      const tokenFunctionMatch = fileContent.match(/getPasswordResetTokenByToken[\s\S]*catch[\s\S]*?return null/);
      expect(tokenFunctionMatch).not.toBeNull();
    });

    it('should return null on error for email function', () => {
      const emailFunctionMatch = fileContent.match(/getPasswordResetTokenByEmail[\s\S]*catch[\s\S]*?return null/);
  ...
  describe('Type Safety', () => {
    ...
    it('should return proper types', () => {
      expect(fileContent).toMatch(/return passwordResetToken/);
  ...
  describe('Data Layer Pattern', () => {
    it('should follow data access naming convention', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenBy/);
    });
    ...
    it('should follow consistent pattern', () => {
      expect(fileContent).toContain('getPasswordResetTokenByToken');
      expect(fileContent).toContain('getPasswordResetTokenByEmail');
    });
  });
  ...
  describe('Return Values', () => {
    it('should return password reset token or null', () => {
      expect(fileContent).toMatch(/return passwordResetToken/);
      expect(fileContent).toMatch(/return null/);
    });
    ...
    it('should consistently return from both functions', () => {
      ...
      expect(returns?.length).toBeGreaterThanOrEqual(4); // 2 returns per function
    });
  ...
  describe('Database Integration', () => {
    ...
    it('should query passwordResetToken model', () => {
      expect(fileContent).toContain('db.passwordResetToken');
    });
  ...
  describe('Password Reset Token Lookups', () => {
    ...
    it('should handle token as unique identifier', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByToken[\s\S]*?findUnique[\s\S]*?token/);
    });

    it('should handle email as searchable field', () => {
      expect(fileContent).toMatch(/getPasswordResetTokenByEmail[\s\S]*?findFirst[\s\S]*?email/);
    });
  });
  ...
  describe('Async/Await Pattern', () => {
    ...
    it('should properly await Prisma calls', () => {
      expect(fileContent).toMatch(/await\s+db\.passwordResetToken\.(findUnique|findFirst)/);
    });
  ...
  describe('Function Purpose', () => {
    it('should retrieve password reset tokens', () => {
      expect(fileContent).toContain('getPasswordResetTokenByToken');
      expect(fileContent).toContain('getPasswordResetTokenByEmail');
    });
    ...
    it('should support password reset flow', () => {
      expect(fileContent).toContain('passwordResetToken');
    });
